...
async function proposeExchange(req, res) {
    ...
    try {
        const propose = await services.exchangeProposal(
            userFrom,
            discsFrom,
            discsTo
        );
        ...
        if (err === "Discos oferecidos não pertencente à usuario")
            return res.status(400).json({ message: err });
        if (err === "Discos pedidos devem pertencer ao mesmo usuário")
            return res.status(400).json({ message: err });
        if (err === "Disco não encontrado")
            return res.status(400).json({ message: err });
        if (err === "Necessário informar ID")
            return res
                .status(400)
                .json({ message: "Necessário informar ID do disco" });
        if (err === "Necessário propor discos")
            return res.status(400).json({ message: err });
        if (err === "Necessário pedir discos")
...
async function accept(req, res) {
    ...
    try {
        ...
        if (err === "Troca não existe")
            return res.status(404).json({ message: err });
        if (err === "Aprovação não pendente")
            return res.status(400).json({ message: err });
        if (err === "Usuário não é dono da troca")
            return res.status(400).json({ message: err });
...
async function cancel(req, res) {
    ...
    try {
        ...
        if (err === "Troca não existe")
            return res.status(404).json({ message: err });
        if (err === "Troca não esta acontecendo")
            return res.status(400).json({ message: err });
        if (err === "Usuário não é dono da troca")
            return res.status(400).json({ message: err });
...
async function complete(req, res) {
    ...
    try {
        const completeRes = await services.completeExchange(
            exchangeId,
            avaliations,
            userId
        );
        ...
        if (err === "Usuário não pertence à troca")
            res.status(403).json({ message: err });
        if (err === "Troca não está acontecento")
            res.status(400).json({ message: err });
        if (err === "Usuário já marcou a troca como completa")
            res.status(400).json({ message: err });
        console.log(err);
